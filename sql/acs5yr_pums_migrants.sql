...
with
    ...
    AS
    (
                    SELECT 2020 AS [year], [ST], [PUMA], [AGEP], [SEX], [HISP], [RAC1P], [MIL], [MIG], [MIGSP], [MIGPUMA], [PWGTP]
            FROM [acs].[pums].[5y_2016_2020_persons]
        UNION ALL
            SELECT 2021 AS [year], [ST], [PUMA], [AGEP], [SEX], [HISP], [RAC1P], [MIL], [MIG], [MIGSP], [MIGPUMA], [PWGTP]
    ...
    (
        SELECT
            ...
            CASE  WHEN [year] BETWEEN 2016 AND 2021
                AND ([MIGSP] != '006' OR ([MIGSP] = '006' AND [MIGPUMA] != '07300')) -- 2010 Census Migration PUMA for San Diego County
                AND [ST] = '06' AND [PUMA] IN (SELECT [PUMA]
                FROM @tt_sd_pumas_2010)  -- 2016-2020 ACS uses 2010 Census PUMAS
				  THEN [PWGTP] ELSE 0 END AS [in],
            CASE  WHEN [year] BETWEEN 2016 AND 2021
                AND ([MIGSP] = '006' AND [MIGPUMA] = '07300') -- 2010 Census Migration PUMA for San Diego County
                AND ([ST] != '06' OR ([ST] = '06' AND [PUMA] NOT IN (SELECT [PUMA]
                FROM @tt_sd_pumas_2010)))
				  THEN [PWGTP] ELSE 0 END AS [out]
        FROM
            [pums_persons]
        WHERE 
