...
with
	...
	(
		SELECT
			...
			CASE WHEN [sex] = 'Female' THEN 'F'
			 WHEN [sex] = 'Male' THEN 'M'
			 ELSE NULL END AS [sex],
			CASE WHEN [age] > 110 THEN 110
             ELSE [age] END AS [age], -- maximum age is 110
			[population] AS [pop]
...
SELECT [vintage], [year], [race], [sex], [age], SUM([pop]) AS [pop]
FROM [formatted_data]
GROUP BY [vintage], [year], [race], [sex], [age]
ORDER BY [vintage], [year], [race], [sex], [age]