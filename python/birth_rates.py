...
# TODO: Add function to allow for input % adjustments to birth rates.
# TODO: Potentially implement smoothing function within race categories.
...
def get_birth_rates(
    base_yr: int, base_df: pd.DataFrame, rates_map: dict
) -> pd.DataFrame:
    ...
    births greater than the total population within the category. Note that no
    inflation factor is made to account for births assigned to under 15 or 45+
    age groups that are excluded.
    ...
    for k, v in rates_map["births"][str(base_yr)].items():
        fp = "data/births/" + str(base_yr) + "/" + v
    ...
    # Merge births with base year population
    merged_births = (
        births.merge(
            right=base_df.loc[
                (base_df["sex"] == "F") & (base_df["age"].between(15, 44))
            ][["race", "age", "pop"]],
            on="race",
        )
    ...
    rates = (
        ...
        .query("age >= age_lower & age <= age_upper")[["race", "sex", "age", "rate"]]
    )

    # If base year is 2010-2017
