...
if data_selector == "CSV":
    ...
    if (
        True in csv_dict["population"]
        and True in csv_dict["components"]
        and True in csv_dict["rates"]
    ):
    ...
    # Write error message if one or more CSVs don't load or exist
        ...
        failed = [
            name
            for name in ["population", "components", "rates"]
...
# Add in section for what to do if "SQL Database" optionn selected
    ...
    if True in metadata:
        ...
        # Logic once run_id is selected
            ...
            sql_dict = report_utils.get_data(
                data_selector=data_selector, run_id=st.session_state.run_id
            )

            # Save tables from SQL Database if they all exist
            if (
                True in sql_dict["population"]
                and True in sql_dict["components"]
                and True in sql_dict["rates"]
            ):
