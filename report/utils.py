...
def plot_line_chart(
    ...
    x: str, 
    y: str, 
    x_range: Optional[List[Union[int, float]]], 
    y_range: Optional[List[Union[int, float]]], 
    color: str, 
    title: str
) -> Figure:
    """Create Line chart.

    This function uses the data given and plots it it with a separate line
    for each category in "color" used in 3_Rates.py

    Args:
        data (pd.DataFrame): Data Frame containing data to be plotted.
        x (str): variable to be plotted on x-axis.
        y (str): variable to be plotted on y-axis.
        x_range (Optional[List[Union[int, float]]]): List to determine to min and max value for the x-axis
        y_range (Optional[List[Union[int, float]]]): List to determine to min and max value for the y-axis
        color (str): variabale to determie the grouping of the lines plotted and assigns each different color
        title: (str): String for Title of the Figure
